...
void move2() {
    ...
    if (abs(x_axis) > when_to_rotate) {
        ...
        {
            new_power_right += power_lift;
            if (new_power_right > new_power_left)
                new_power_right = new_power_left;
            delay(50);
        }
        else if (new_power_left < new_power_right)
        {
            new_power_left += power_lift;
            if (new_power_right < new_power_left)
                new_power_left = new_power_right;
            delay(50);
        }
    }
...
void self_rotate() {
    ...
    if (abs(x_axis) > when_to_rotate)
    {
        if (x_axis > 0 && !is_rotate_left)
        {
            ...
            if (new_power_left > self_rotate_gap)
                new_power_left = self_rotate_gap;
            if (new_power_right > self_rotate_gap)
                new_power_right = self_rotate_gap;
            right_pin = MOTOR_PIN[RIGHT][MAX_GEAR - 1];
    ...
    if ((is_rotate_left || is_rotate_right) && new_power_left == new_power_right && new_power_left == 0)
    {
